...
SELECT player FROM goal JOIN game ON (matchid = game.id)
WHERE stadium = 'National Stadium, Warsaw'

-- 8. The example query shows all goals scored in the Germany-Greece quarterfinal.
-- Instead show the name of all players who scored a goal against Germany.

SELECT DISTINCT player FROM goal JOIN game ON (matchid = game.id)
WHERE teamid <> 'GER' AND (team1 = 'GER' OR team2 = 'GER')

-- 9. Show teamname and the total number of goals scored.

SELECT teamname, COUNT(*) FROM goal JOIN eteam ON (eteam.id = teamid)
GROUP BY teamname

-- 10. Show the stadium and the number of goals scored in each stadium.

SELECT stadium, COUNT(*) FROM goal JOIN game ON (matchid = game.id)
GROUP BY stadium

-- 11. For every match involving 'POL', show the matchid, date and the number of goals scored.

SELECT matchid, mdate, COUNT(*) FROM goal JOIN game ON (matchid = game.id)
...
SELECT mdate, team1, (SELECT COUNT(*) FROM goal WHERE teamid = team1 AND game.id = matchid), team2, (SELECT COUNT(*) FROM goal WHERE teamid = team2 AND game.id = matchid) FROM game
ORDER BY mdate