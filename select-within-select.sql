...
WHERE area >= ALL(SELECT area FROM world WHERE continent = x.continent)

-- 8. List each continent and the name of the country that comes first alphabetically.

SELECT continent, name FROM world x
WHERE name <= ALL(SELECT name FROM world WHERE x.continent = continent)
...
WHERE population >= ANY(SELECT MAX(population)*3 FROM world WHERE x.continent = continent AND x.name <> name)